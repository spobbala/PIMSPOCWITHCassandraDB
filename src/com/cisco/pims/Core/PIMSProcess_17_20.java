...
//					while(rSet1.next()){
					pstmt = DBConnectionFactory.prepareStatement(pimsCon,
							PIMSConstants.DELETEQUERY_17_20, mValues.get("SN"));
					pstmt.executeUpdate();
//					}
					pstmt = DBConnectionFactory.prepareStatement(pimsCon,
					...
					String mfgDate = mValues.get("MFGDATE");
					DateFormat dfm = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");  
					Date dd = dfm.parse(mfgDate);
					Timestamp javaTimestamp = new Timestamp(dd.getTime());
					pstmt.setTimestamp(6, javaTimestamp);
...
//				pimsCon.rollback();
	...
	private String getCustID(String shipToID) throws SQLException {
		...
		pstmt = DBConnectionFactory.prepareStatement(pimsCon,
				PIMSConstants.QUERYDNCSGET, shipToID);
		rSet = pstmt.executeQuery();
		
		while (rSet.next())
			dncsID = rSet.getString("dncs_id");
		pstmt = DBConnectionFactory.prepareStatement(pimsCon,
				PIMSConstants.QUERYCAAGET, dncsID);
		rSet = pstmt.executeQuery();
		while(rSet.next())
