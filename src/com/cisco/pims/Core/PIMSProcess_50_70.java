...
//		this.writeHctTypes(parentFolder, tempDir, batchid);
	...
	private Map<String, Map<String, byte[]>> generateBlobs(int batchid) {
		...
		try {
			...
			while(rSet1.next()){
			pstmt = DBConnectionFactory.prepareStatement(pimsCon,
					PIMSConstants.QUERYPROD50_70, rSet1.getString("dhct_sn"));
			rSet = pstmt.executeQuery();
			while (rSet.next()) {
				if(rSet1.getString("pallet_id")!=null && rSet1.getString("pallet_id").length() > 0 && cnt==0)
				...
				pstmt = DBConnectionFactory.prepareStatement(pimsCon,
						PIMSConstants.QUERYEBS40_50, rSet.getString("mfg_id"), rSet.getString("model"),
				...
				if (rSet1.getString("pallet_id") != null && rSet1.getString("pallet_id").length() > 0) {
					...
					if (tocPalletBlob.get(rSet1.getString("pallet_id")) == null)
						tocPalletBlob.put(rSet1.getString("pallet_id"), tocBlob);
					else {
						...
						System.arraycopy(
								tocPalletBlob.get(rSet1.getString("pallet_id")),
								0,
								newBlob,
								0,
								tocPalletBlob.get(rSet1.getString("pallet_id")).length);
						System.arraycopy(
								tocBlob,
								0,
								newBlob,
								tocPalletBlob.get(rSet1.getString("pallet_id")).length,
								tocBlob.length);
						tocPalletBlob.put(rSet1.getString("pallet_id"), newBlob);
					}
				}
				...
				if (rSet1.getString("pallet_id") != null && rSet1.getString("pallet_id").length() > 0) {
					if (invPalletBlob.get(rSet1.getString("pallet_id")) == null)
						...
						byte[] newBlob = new byte[len];
						System.arraycopy(
								invPalletBlob.get(rSet1.getString("pallet_id")),
								0,
								newBlob,
								0,
								invPalletBlob.get(rSet1.getString("pallet_id")).length);
						System.arraycopy(
								invBlob,
								0,
								newBlob,
								invPalletBlob.get(rSet1.getString("pallet_id")).length,
								invBlob.length);
						invPalletBlob.put(rSet1.getString("pallet_id"), newBlob);
					}
				}
