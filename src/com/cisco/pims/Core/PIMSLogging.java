...
public class PIMSLogging {
	...
	public void logMessage(int iBatchId, String iDhctSn, String iItemNo,
		...
		try {
			errorDump = null; //(messageId == getErrMsgId()) ? getErrorReport(iBatchId) : null;
			pstmt = DBConnectionFactory.prepareStatement(dbCon,
					PIMSConstants.LOGUPDATEQUERY);
//			, iBatchId, iDhctSn, iItemNo,
//					iSeqNo, messageType, messageId, iMessageDetails, errorDump);
			UUID uuid = new java.util.UUID(UUIDGen.newTime(), UUIDGen.getClockSeqAndNode());
		    byte[] uuidByte = PIMSHelper.getUniqueTimeUUIDinMillis(uuid);
			this.setNotificationID(uuid.toString());
		    this.addNotificationID(uuid.toString());

			pstmt.setBytes(1,uuidByte);
			pstmt.setInt(2, iBatchId);
			pstmt.setString(3, iDhctSn);
			pstmt.setString(4, iItemNo);
			pstmt.setInt(5, iSeqNo);
			pstmt.setString(6, messageType);
//			pstmt.setTimestamp(6, timestamp);
			pstmt.setInt(7, messageId);
			pstmt.setString(8, iMessageDetails);
			pstmt.executeUpdate();
//			dbCon.commit();
